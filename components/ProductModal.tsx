...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  if (!portalRoot) {
      console.error("The portal root element was not found in the DOM.");
      return null;
  }
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div 
        className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up" 
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-4 flex justify-between items-center border-b border-slate-200 dark:border-slate-700 shrink-0">
          <h2 id="product-modal-title" className="text-lg sm:text-xl font-extrabold">{product.name[language]}</h2>
