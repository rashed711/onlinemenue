        ...
        <>
            <Modal title={`${t.orderDetails} - #${order.id.substring(order.id.length - 6)}`} onClose={onClose} size="4xl">
                <div className="flex flex-col overflow-hidden h-full">
                    <div className="p-4 sm:p-6 overflow-y-auto flex-grow flex flex-col gap-6 bg-slate-50 dark:bg-slate-900/50">
                        {/* New Top Info Box */}
                        <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-y-6 md:gap-x-6">
                                ...
                                {/* Customer Info - last column in LTR, first in RTL */}
                                <div className="space-y-1 md:order-3 text-start rtl:text-end md:text-end rtl:md:text-start">
                                     <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-2">{t.customerInfo}</h3>
                                     <p className="font-semibold text-base text-slate-800 dark:text-slate-200">{order.customer.name || 'Guest'}</p>
                                     <p className="font-mono text-sm text-slate-600 dark:text-slate-300">{order.customer.mobile}</p>
                                     {order.customer.address && (
                                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 whitespace-pre-wrap">{order.customer.address}</p>
                                    )}
                                </div>

                                {/* Order Details - middle column */}
                                <div className="space-y-1 md:text-center self-start md:order-2">
                                    <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-2">{t.orderDetails}</h3>
                                    <p className="font-semibold text-base text-slate-700 dark:text-slate-200">
                                        <span className="text-sm font-medium text-slate-500 dark:text-slate-400">{t.orderType}: </span>
                                        {t[order.orderType.toLowerCase() as keyof typeof t]} {order.tableNumber && `(${t.table} ${order.tableNumber})`}
                                    </p>
                                </div>

                                {/* Order Info - first column */}
                                <div className="space-y-4 md:order-1 text-start rtl:text-end">
                                    <div>
                                        <p className="font-semibold text-sm text-slate-500 dark:text-slate-400">{t.orderId}</p>
                                        <p className="font-mono text-base text-slate-700 dark:text-slate-200">{order.id}</p>
                                    </div>
                                    <div>
                                        <p className="font-semibold text-sm text-slate-500 dark:text-slate-400">{t.date}</p>
                                        <p className="text-base text-slate-700 dark:text-slate-200">{formatDateTime(order.timestamp)}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                             <div className="flex justify-between items-center mb-3">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className={`inline-block px-2.5 py-0.5 text-xs font-bold rounded-full border ${getStatusChipColor(order.status, restaurantInfo)}`}>
