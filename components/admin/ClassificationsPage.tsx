...
export const ClassificationsPage: React.FC<ClassificationsPageProps> = (props) => {
    const { language, categories, tags, onAddCategory, onEditCategory, onDeleteCategory, onAddTag, onEditTag, onDeleteTag } = props;
                            ...
                            {categories.map(category => (
                                <li key={category.id} className="p-4 flex justify-between items-center hover:bg-slate-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onEditCategory(category)} className="p-2 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-full"><PencilIcon className="w-5 h-5" /></button>
                                        <button onClick={() => onDeleteCategory(category.id)} className="p-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full"><TrashIcon className="w-5 h-5" /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
                         ...
                         <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                            {tags.map(tag => (
                                <li key={tag.id} className="p-4 flex justify-between items-center hover:bg-slate-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onEditTag(tag)} className="p-2 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-full"><PencilIcon className="w-5 h-5" /></button>
                                        <button onClick={() => onDeleteTag(tag.id)} className="p-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full"><TrashIcon className="w-5 h-5" /></button>
                                    </div>
                                </li>
                            ))}
                        </ul>
