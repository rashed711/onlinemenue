...
export const InventoryPage: React.FC = () => {
    ...
    const { suppliers, purchaseInvoices, addSalesInvoice, salesInvoices, deleteSupplier, deletePurchaseInvoice, deleteSalesInvoice } = useAdmin();
        ...
        <div>
           <div className="flex justify-end mb-4">
               {canManageSalesInvoices && (
                   <button onClick={() => setIsSalesInvoiceModalOpen(true)} className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 transition-colors flex items-center gap-2">
                       <PlusIcon className="w-5 h-5" />
    ...
    return (
        <div className="animate-fade-in-up">
                ...
                <nav className="-mb-px flex space-x-6 rtl:space-x-reverse" aria-label="Tabs">
                    ...
                    {canManageSalesInvoices && (
                         <button
                            ...
                            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'salesInvoices' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:border-slate-600'}`}
                        >
                            <CashRegisterIcon className="w-5 h-5"/> {t.salesInvoices}
                        </button>
                    )}
                </nav>
            </div>
            ...
            {isSalesInvoiceModalOpen && (
                <SalesInvoiceModal onClose={() => setIsSalesInvoiceModalOpen(false)} />
            )}
             {viewingSalesInvoice && (
