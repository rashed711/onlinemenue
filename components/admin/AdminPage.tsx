...
export const AdminPage: React.FC<AdminPageProps> = ({ activeSubRoute, reportSubRoute }) => {
    ...
    const sortedCategories = useMemo(() => {
        ...
        const categoriesMap = new Map<number, CategoryNode>(categories.map(c => [c.id, { ...c, children: [] }]));
        ...
        // Sort roots by their sort_order
        roots.sort((a, b) => a.sort_order - b.sort_order);
        
        const sorter = (a: Category, b: Category) => a.name[language].localeCompare(b.name[language], language);
    ...
    const renderContent = () => {
        switch(displayedTab) {
            ...
            case 'productList':
                ...
                const categoryFilterClasses = openCategoryDropdown !== null
                    ? "flex items-start gap-2 py-2 flex-wrap"
                    : "flex items-start gap-2 py-2 overflow-x-auto scrollbar-hide flex-nowrap";
                return (
                    <div className="flex flex-col h-[calc(100vh-6.5rem)]">
                        ...
                        <div className="shrink-0 bg-white dark:bg-slate-800 rounded-xl shadow-lg mb-6 border border-slate-200 dark:border-slate-700">
                            <div className="p-4 space-y-4">
                                <div className="flex flex-row items-start gap-2">
                                    <div className="relative w-full flex-grow">
                                        <label htmlFor="product-search" className="sr-only">{t.search}</label>
                                        <input 
                                            id="product-search"
                                            type="text" 
                                            placeholder={`${t.productNameEn}, ${t.code}...`} 
                                        ...
                                        />
                                        <div className="absolute top-1/2 -translate-y-1/2 start-3 text-slate-400 pointer-events-none"><SearchIcon className="w-5 h-5" /></div>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setIsTagFilterOpen(!isTagFilterOpen)}
                                        className={`flex-shrink-0 flex items-center justify-center gap-2 px-3 py-2 border rounded-lg font-bold text-sm transition-colors ${isTagFilterOpen ? 'bg-primary-100 dark:bg-primary-900/50 border-primary-500 text-primary-700 dark:text-primary-300' : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:border-primary-400 dark:hover:border-primary-500'}`}
                                    >
                                        <FilterIcon className="w-5 h-5" />
                                        <span>{t.tags}</span>
                                        {productFilterTags.length > 0 && (
                                            <span className="bg-primary-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                                                {formatNumber(productFilterTags.length)}
                                            </span>
                                        )}
                                    </button>
                                </div>

                                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isTagFilterOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <div className={categoryFilterClasses}>
                                            <button
