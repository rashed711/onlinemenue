...
export const SalesInvoiceDetailsModal: React.FC<SalesInvoiceDetailsModalProps> = ({ invoice, onClose }) => {
    const { t, language } = useUI();
    
    return (
        <Modal title={`${t.invoiceDetails} - #${invoice.invoice_number}`} onClose={onClose} size="2xl">
            <div className="p-5 space-y-6 max-h-[80vh] overflow-y-auto">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <p className="font-semibold text-slate-800 dark:text-slate-100">{invoice.customer_name} ({invoice.customer_mobile})</p>
                    </div>
                     <div>
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                {invoice.items.map((item, index) => (
                                    <tr key={item.id || index}>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-end text-slate-600 dark:text-slate-300">{item.sale_price.toFixed(2)}</td>
