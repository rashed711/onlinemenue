...
import type { Language, User, Order, RestaurantInfo } from '../../types';
...
export const ProfilePage: React.FC<ProfilePageProps> = (props) => {
    const { language, currentUser, orders, restaurantInfo, logout, onChangePasswordClick, onUpdateProfile, updateOrder } = props;
    ...
    const OrderStatusTracker: React.FC<{ order: Order }> = ({ order }) => {
        ...
        if (currentStatusIndex === -1) {
            return null; // Don't show tracker for cancelled/refused orders
        }
            ...
            <div className="flex items-center justify-between">
                {statuses.map((status, index) => {
                    ...
                    const stateClasses = {
                        ...
                        future: 'bg-slate-300 dark:bg-slate-600 border-slate-300 dark:border-slate-600'
                    };
                    
                    let currentState = 'future';
                    if (isCompleted) currentState = 'completed';
                    if (isCurrent) currentState = 'current';
                    ...
                    return (
                        <React.Fragment key={status.id}>
                            <div className="flex flex-col items-center text-center">
                                ...
                                <p className={`mt-2 text-xs font-semibold ${isCurrent ? 'text-blue-600 dark:text-blue-300' : 'text-slate-600 dark:text-slate-400'}`}>
                                    {status.name[language]}
                                </p>
                            </div>
                            {index < statuses.length - 1 && (
                                <div className={`flex-1 h-1 mx-2 rounded ${isCompleted ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
                            )}
                        </React.Fragment>
                    );
                })}
                    ...
                    <div className="lg:col-span-1 space-y-8 sticky top-24">
                        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6 sm:p-8 text-center animate-fade-in-up">
                             <div className="relative w-32 h-32 mx-auto mb-4">
                                ...
                                <button onClick={() => profilePicInputRef.current?.click()} className="absolute bottom-0 end-0 bg-primary-500 text-white rounded-full p-2 hover:bg-primary-600 transition-transform transform hover:scale-110 shadow-md" aria-label={t.changeProfilePicture} title={t.changeProfilePicture}>
                            ...
                            {!isEditingName ? (
                                <div className="flex items-center justify-center gap-2">
                                    ...
                                    <button onClick={() => setIsEditingName(true)} className="text-slate-500 hover:text-primary-600 p-1"><PencilIcon className="w-5 h-5" /></button>
                                </div>
                            ) : (
                         ...
                         <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6 sm:p-8 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                            ...
                            <div className="space-y-4">
                                <button onClick={onChangePasswordClick} className="w-full flex justify-between items-center text-start p-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-700/50 dark:hover:bg-slate-700 transition-colors">
                             ...
                             <div className="animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                                ...
                                <div className="space-y-6">
                                    {activeOrders.map(order => (
                                        <div key={order.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6">
                                    ...
                                    <div key={order.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-4">
                                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                            <div className="flex-1 space-y-1">
                                                ...
                                                <p className="text-sm text-slate-500">{order.items.length} items - <span className="font-bold">{order.total.toFixed(2)} {t.currency}</span></p>
                                                {order.orderType === 'Delivery' && order.customer.address && (
                                                    <p className="text-xs text-slate-500 pt-1 border-t border-slate-200 dark:border-slate-700 mt-2">{t.address}: {order.customer.address}</p>
                                                )}
                                            </div>
                                            <div className="flex flex-col items-start sm:items-end gap-2 w-full sm:w-auto">
                                                ...
                                                {order.status === 'completed' && (
                                                    order.customerFeedback ? (
                                                        ...
                                                        <button onClick={() => setFeedbackOrder(order)} className="text-sm font-semibold text-primary-600 hover:underline">{t.leaveFeedback}</button>
                                                    )
                                                )}
                                            </div>
                                        </div>
