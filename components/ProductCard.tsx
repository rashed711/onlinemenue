  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 h-12 line-clamp-2">{product.name[language]}</h3>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 flex-grow line-clamp-2">{product.description[language]}</p>
        
        <div className="flex justify-between items-center mt-4">
          <p className="text-xl font-extrabold text-primary-600 dark:text-primary-400">
            {product.price.toFixed(2)} <span className="text-sm font-semibold">{t.currency}</span>
          </p>
       ...
       <div className="p-4 pt-0">
         <button
          ...
          className="w-full bg-primary-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-primary-600 flex items-center justify-center gap-2 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-slate-800 shadow-sm hover:shadow-lg transform hover:scale-105 text-xs sm:text-sm"
        >
