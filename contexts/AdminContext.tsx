...
export const AdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const updateProduct = useCallback(async (updatedProduct: Product, imageFile?: File | null) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                const oldProduct = originalProducts.find(p => p.id === updatedProduct.id);
                if (oldProduct && oldProduct.image) {
                    formData.append('oldPath', getRelativePath(oldProduct.image));
                }
                const uploadRes = await fetch(`${API_BASE_URL}upload_image.php`, { method: 'POST', body: formData });
                const result = await uploadRes.json();
                if (!uploadRes.ok || !result.success) throw new Error(result.error || 'Image upload failed');
    ...
    const updateUser = useCallback(async (updatedUser: User) => {
        ...
        try {
            ...
            if (currentUser?.id === updatedUser.id) {
                ...
                setCurrentUser(originalSelf || null);
            }
        } finally {
            setIsProcessing(false);
        }
    ...
    const updateCategoryOrder = useCallback(async (orderedIds: number[]) => {
        ...
        // --- Optimistic Update ---
        const optimisticallyUpdatedCategories = originalCategories.map(cat => {
            const newIndex = orderedIds.indexOf(cat.id);
            if (!cat.parent_id && newIndex !== -1) {
                return { ...cat, sort_order: newIndex };
            }
            return cat;
        });
        setCategories(optimisticallyUpdatedCategories);
        // --- End Optimistic Update ---

        setIsProcessing(true);
        try {
            const response = await fetch(`${API_BASE_URL}update_category_order.php`, {
                method: 'POST',
            ...
            const result = await response.json();
            if (!response.ok || !result.success) {
                throw new Error(result.error || t.categoryOrderUpdateFailed);
            }
            showToast(t.categoryOrderUpdatedSuccess);
            ...
            showToast(error.message || t.categoryOrderUpdateFailed);
            setCategories(originalCategories);
        } finally {
            setIsProcessing(false);
        }
    }, [hasPermission, t, categories, setCategories, setIsProcessing, showToast]);
