...
const getRelativePath = (url: string): string => {
    if (!url || !url.startsWith('http')) {
        return url.split('?v=')[0]; // It's already relative or empty, remove version query param
    }
    try {
        const urlObject = new URL(url);
        const path = urlObject.pathname;
        // The path will be like /uploads/branding/....png. We need to remove the leading slash.
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchAllData = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            const fetchOptions = { method: 'GET', cache: 'no-cache' as RequestCache };
    ...
    const updateRestaurantInfo = useCallback(async (updatedInfo: Partial<RestaurantInfo>) => {
        ...
        try {
            const uploadImage = async (base64: string, type: string, field: string, oldPath?: string): Promise<string | null> => {
                if (!base64 || !base64.startsWith('data:image')) {
                    return getRelativePath(base64);
                }
            ...
            for (const key of Object.keys(updatedInfo) as Array<keyof RestaurantInfo>) {
                ...
                switch (key) {
                    ...
                    case 'logo': case 'heroImage':
                        if ((value as string).startsWith('data:image')) {
                            const oldImagePath = getRelativePath(restaurantInfo[key]);
                            const uploadResult = await uploadImage(value as string, 'branding', key, oldImagePath);
                            
                            if (uploadResult !== null) {
                                dbPayload[key] = uploadResult.split('?v=')[0];
                                (uiUpdates as any)[key] = resolveImageUrl(uploadResult);
                            }
                        }
                        break;
                    case 'socialLinks':
                    case 'onlinePaymentMethods':
                        ...
                        const oldItems = restaurantInfo[key] as any[] || [];
                        const type = key === 'socialLinks' ? 'icons' : 'payment_icons';
                        const processedItems = await Promise.all(
                            items.map(async (item, i) => {
                                if (item.icon && item.icon.startsWith('data:image')) {
                                    const oldItem = oldItems.find(oi => oi.id === item.id);
                                    const oldIconPath = oldItem ? getRelativePath(oldItem.icon) : undefined;
                                    const uploadResult = await uploadImage(item.icon, type, `${key}_${item.id || i}_icon`, oldIconPath);
                                    const relativeUrl = uploadResult ? uploadResult.split('?v=')[0] : item.icon;
                                    return { ...item, icon: relativeUrl };
                                }
                                return { ...item, icon: getRelativePath(item.icon) };
                            })
                        );
